...
\section*{Introduction} % about 4K plus 730 in boxes (4,550 words currently and by Jan 2024: Up to 5K ... feels a little too long to me, down to 4840 on 21 Feb 2024 ... down to 4K on 9 March ... but I assume that did not include boxes) on 24 June 2024: 3764 K plus 176 in one box (I got rid of a box).
...
\subsection*{Species-level variation} %TODO -- add species variation in leaf versus wood growth and temporal lags here? Also, add that phylo can stand in for TRAITS... 
...
%% STARTING new text
\subsection*{NEW: Species-level variation}. 
The effects of these external and internal drivers are likely to vary across species, with species identity strongly predicting variation in growth $\times$ season length relationships \citep[e.g.][]{cuny2012life,michelot2012comparing}. Though this reality was rarely mentioned in studies we reviewed (Fig. \ref{fig:hypotheses}c), research in dendrochronology, physiology and in phenology often acknowledges important differences between certain species groups that should affect how longer seasons affect growth. The distinct strategies of deciduous versus evergreen species, including in how and when they invest in leaf and shoot elongation versus cambial growth, can affect how they respond to longer seasons. While evergreen species generally leafout later than deciduous species they can more immediately photosynthesize with earlier springs, though both types of species generally invest in buds for new leaves, shoots and flowers in the preceding year, meaning neither can rapidly change its leaf area in response to an earlier spring, and both types can have multiple flushes of leaves CITES. Wood growth in evergreen species is generally thought to come from current season photosynthesate, while deciduous species may more often use stored carbon resources (CITES). These differences would suggest season length by growth relationships may be more apparent in evergreen species, and more apparent via lagged effects in deciduous species CITES, though to date this has not been consistently reported, but evergreen species are also thought to grow more slowly and thus differences may be harder to detect CITES.  %CHECKREFSandINFO

This division between evergreen and deciduous species hint at a larger suite of traits that predict growth by season length relationships among species. Species that budburst earlier and more readily produce additional leaves (e.g., lammas growth and other characteristics of `indeterminate' species) that more readily respond to longer seasons versus those that budburst later and flush new primary growth only once. Similarly, species adapted to cold or dry conditions across their range may have different thresholds for when these external drivers limit or promote growth CITES. Such differences could easily obscure any overall relationship between growth and growing season length. Supporting this possibility, current studies span a wide range of species (we found  \Sexpr{sppnum} species from \Sexpr{gennum} genera across \Sexpr{papernum} papers), and have found generally divergent results (CITEFIG). % making the aim of identifying a common relationship between growth and growing season length with current studies especially difficult.

\section*{NEW version of: Building a new framework for growth $\times$ season length relationships} 
...
Leveraging the diversity of responses observed across species is possible by integrating ecological advances in understanding how species traits and evolutionary history shape species responses to climate change CITES. In particular, ecophylogenetics has moved research away from treating species identity as a simple grouping factor where each species is unique (e.g., \emph{Fagus sylviatica} is different from \emph{Quercus robur} and \emph{Pinus sylvestris}) or fits into a limited set of groups (e.g., deciduous versus evergreen) and towards species as related based on the evolutionary distance (e.g., \emph{Fagus sylviatica} is much more closely related to \emph{Quercus robur} compared to \emph{Pinus sylvestris}, CITES), producing models that can fit data from all species at once, and leverage differences in growth strategies through additionally measured traits. Leaf and wood economics from trait ecology suggest dominant traits that could first be build into these models (with related databases of these traits often available, CITES). These `economics' define a common trade-off along an acquisitive to conservative axis, where some species grow rapidly and more flexibly to take advantage of resources, but are less defended against herbivores and compete poorly at low resource levels, whereas other species compete well at low resource levels, but at the expense of growing slower and conservatively \citep[][]{Grime:1977sw}. While these traits likely miss critical components such as when different species invest in shoot and leaf versus wood growth, they provide a baseline framework from which to build and a way to combine data usefully across species. This approach aids the search for a universal model of how growing season length influences growth by providing a framework that effectively assumes one model, which is then shaped by evolutionary history and trait differences to produce the divergent responses observed across species today. 
...
In addition to naturally organizing species differences, a trait-based ecophylogenetic approach can help build a more testable and predictable framework. % that can more readily forecast species responses ... 
Because this approach can fit evolutionary history and traits together, it allows clades or species groupings that respond similarly to emerge from the data and models, versus being a priori grouped or defined (do we need an EXAMPLE?) CITES. Similarly traits that co-vary with different responses can be more quickly identified CITES. Both of these benefits could help identify which species or traits to focus additional studies on to gain the most insights, while similarly suggesting areas that should be less studied (e.g., traits that may be too confounded with evolutionary history) CITES. This approach may thus redefine debates over which metrics of growth or growing season length are relevant into debates over which metrics are most relevant for which clades and/or traits (add an EXAMPLE?). It also provides an organizing method to re-approach the fundamental question of how external and internal drivers shape growth responses to longer seasons, and a way to tackle what we are argue the three most critical questions in this area. 
...
The idea that growing season length influences plant growth is fundamental to plant biology, but we found it is rarely tested in ways relevant to the current debate (CITE fig or SUPP?), providing a major gap that limits progress. For example, while multiple papers report a lack of relationship between growth and growing season length (CITEFIG), we have no fundamental understanding of what the effect size of this relationship should be, and thus no way to know if we have sufficient power in current studies to detect it. Additionally, understanding large-scale pattens often improves fundamental models of processes, by finding unexpected trends or highlighting covarying drivers that must be disentangled. % ; for example, it may force a more immediate efforts to tease out effects of growth rates versus longer seasons. 

Identifying the macro-scale pattern of how growth and growing season length relate should be a primary goal, and one that seems broadly tractable. Tree ring studies designed to leverage latitudinal and elevational gradients in climate could quickly provide the raw data. Research will then will need to develop models that tease out the effects of warmer temperatures across the season---likely affecting important biological rates (CITEMORAFIG?)---versus longer seasons. Disentangling these may require focused efforts to understand xylogenesis across species and climates, but these doing so across major climatic gradients could make differences more obvious. Research will also then need to integrate beyond wood growth, but wood growth would provide a baseline from which to set expectations of much growth can vary across space and and also time, since tree rings capture inter-annual variation, where studies to date have focused. 
...
New evidence suggests inter-annual variation growth may be limited because of internal constraints that prevent plants from fully using longer seasons CITES. If true, this would have major ramifications for forecasts of shifts in tree growth with warming and fundamental plant biology, but requires much more understanding. Certainly, all plants are limited by internal constraints and how quickly they can build new tissues but selection towards different growth strategies (e.g., acquisitive versus conservative) should drive variation in these constraints across species, and possibly populations. As budset---which indicates the end of height growth---is strongly locally adapted, we should expect variation in these constraints across population, as selection should favor individuals better matches to local environmental optima CITES. 

Current results generally schmear across species and populations (e.g., using NDVI CITES), but new studies could rapidly test for constraints across species and populations work towards a predictive framework using phylogeny and traits to predict these constraints. %For example, latitudinal or elevational studies can also model effects due to the local site, which may correlate with population-level constraints.  
Studies across space could provide early inferences by studying how phenological and wood seasons vary---and attributing variation through models that nest species within populations and include traits, while experiments could provide more robust tests. While juvenile stages of trees are often more flexible than their adult forms, they still often provide predictable inference in differences across species and populations, and so should be integrated far more into studies of how season length affects growth. Using saplings and controlled environments could quickly test how much growth can shift with longer season---providing a potentially standardized way to compare constraints across species and populations. Such studies could also begin to tease out effects of increased growth rates due to warmer temperatures versus longer seasons. 
...
% START HERE ... next section

\subsection*{How do external drivers and internal constraints act together (or: how do they compare?)?}
