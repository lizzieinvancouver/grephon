...
\author{E.M. Wolkovich$^1$, Ailene K. Ettinger$^2$, Alana Chin$^3$, Catherine J. Chamberlain$^4$,\\ Frederik Baumgarten$^1$, Kavya Pradhan$^5$, Rub{\'e}n D. Manzanedo$^{6,7}$ \& Janneke Hille Ris Lambers$^6$}
\date{\today}
\maketitle
...
$^5$  \\ 
$^6$  Institute for Integrative Biology, ETH Zurich, 8092 Zurich, Switzerland \\ %
...
\section*{Introduction} % about 4K plus 730 in boxes (4,550 words currently and by Jan 2024: Up to 5K ... feels a little too long to me, down to 4840 on 21 Feb 2024 ... down to 4K on 9 March ... but I assume that did not include boxes) on 29 May 2024: 3800 K plus 180 in one box (I got rid of a box).
...
\caption{Pathways through which climate change could alter growing season length and growth, with the number of papers we found mentioning each hypothesis from a review of growth $\times$ growing season length relationship studies (for more details, see Supplement).} % TRY to add source/sink maybe? % Hypotheses focus on both source (photosynthesis-limited, including $CO_2$ limitation) and sink limitations ....
...
\caption{Growth $\times$ elevation relationships from the literature with simple linear regression fits shown with 89\% confidence intervals. \citet{oleksyn1998growth} measured growth (mm) as diameter at breast height increments, while the other studies \citep{wang2017climatic,zhou2022altitudinal} measured it as ring width. See Supplement for more methods details.} % and Fig. \ref{fig:growelevMORA} for an example from one site.
\label{fig:gxelev}
\end{figure}
